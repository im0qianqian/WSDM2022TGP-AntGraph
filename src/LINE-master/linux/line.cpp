...
struct Edge{
	...
	int label;
	Edge(){
		src_id = dst_id = label = -1;
	}
	Edge(int src_id, int dst_id, int label):src_id(src_id), dst_id(dst_id), label(label){}
}*all_sampling_list;
...
long long real_total_samples = 0;	// 实际生成的 sampling 数目，total_samples 是指定的数目
...
void ReadData()
{
	...
	for (int k = 0; k != num_edges; k++)
	{
		...
		// 在这里记录这条边已存在于图上，单线程安全
		graph[edge_source_id[k]].insert(edge_target_id[k]);
	}
	fclose(fin);
...
void InitDiy(){
	real_total_samples = (total_samples / num_threads + 3) * num_threads * (num_negative + 1);
	all_sampling_list = (Edge*) malloc(sizeof(Edge) * real_total_samples);
	graph.clear();
}
...
void *TrainLINEThread(void *id)
{
	...
	long long count = 0, last_count = 0, curedge, count_max = total_samples / num_threads + 3;
	long long record_start_idx = (long long)id * count_max * (num_negative + 1);
	...
	for (count = 0; count < count_max; count++)
	{
		...
		for (int d = 0; d != num_negative + 1; d++)
		{
			...
			else
			{
				...
				// 这里校验 (u, target) 是否在图中出现，若出现则重新采样，多线程读安全
//				if(graph[u].count(target)){
//					// 要是完全图可能出问题
//
//                    if(rand() * 1.0 / RAND_MAX > 0.875){
//                    	--d;
//					    continue;
//                    }
//				}
			...
			assert(record_start_idx + count * (num_negative + 1) + d < real_total_samples);
			all_sampling_list[record_start_idx + count * (num_negative + 1) + d] = Edge(u, target, label);
		}
		for (int c = 0; c != dim; c++) emb_vertex[c + lu] += vec_error[c];
	}
	// printf("id: %lld, count: %lld, tt: %lld, st: %lld, ed: %lld\n", (long long)id, count,
	//             real_total_samples,
	//             record_start_idx,
	//             record_start_idx + count * (num_negative + 1) - 1);
	free(vec_error);
	pthread_exit(NULL);
}
...
void OutputEdgeSamplingRes(){
	// 输出 edge sampling result
	char filepath[128] = {0};
	strcpy(filepath, embedding_file);
	strcpy(filepath + strlen(embedding_file), ".edge_sampling");
	printf("edge sampling filepath: %s\n", filepath);

	FILE *fo = fopen(filepath, "wb");
	for (int i = 0; i < real_total_samples; i++){
		int src_id = all_sampling_list[i].src_id;
		int dst_id = all_sampling_list[i].dst_id;
		int label = all_sampling_list[i].label;
		fprintf(fo, "%s %s %d\n", vertex[src_id].name, vertex[dst_id].name, label);
	}
	fclose(fo);
}
