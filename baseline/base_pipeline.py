...
class HeteroConv(nn.Module):
    ...
    def build_hconv(self, in_feats, out_feats, activation=None):
        ...
        for event_type in self.etypes:
            GNN_dict[event_type] = dglnn.SAGEConv(in_feats=in_feats, out_feats=out_feats, aggregator_type='mean',
...
def test(args, g, model):
    model.eval()
    test_csv = pd.read_csv(f'test_csvs/input_{args.dataset}_initial.csv',
